...
export default function ResetPassword() {
  ...
  let countError={};
  function Validation(){
if(!newPassword.updatePassword || !passwordRegex.test(newPassword.updatePassword)){
    countError.password = "Password must be at least 8 characters with uppercase, lowercase, a number, and a special character.";
  }
  setError(countError);
 ...
 async function HandleSubmit(e){
  ...
  return (
    <form  onSubmit={(e)=>HandleSubmit(e)} className=" h-screen  w-screen flex flex-col justify-center items-center gap-8">
     <div className=" h-1/5 w-full flex flex-col items-center justify-center"> <input
        type="password"
        name="updatePassword"
      ...
      type="submit"
       className="w-[30%] bg-[#37B6FF] h-[8%] rounded-full font-semibold text-center text-white will-change-transform duration-100 ease-out hover:scale-105 shadow-sm">
        {loader ? (<div className="h-5 w-5 rounded-full border-2 border-white border-t-transparent animate-spin"></div>) : "Request Password change"}
      </button>
    </form>
  );
}
